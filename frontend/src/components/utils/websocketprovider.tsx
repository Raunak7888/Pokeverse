...
export const WebSocketProvider: React.FC<{ 
  ...
  useEffect(() => {
    ...
    const subscribeToTopic = (topic: string) => {
      ...
      console.log(`✅ Subscribed to ${topic} (${subscriptionsRef.current.get(topic)?.size || 0} listeners)`);
    };
  ...
  const subscribe = useCallback((topic: string, callback: (msg: IMessage) => void) => {
    ...
    if (clientRef.current?.connected && !activeStompSubsRef.current.has(topic)) {
      const stompSub = clientRef.current.subscribe(topic, (message) => {
        const callbacks = subscriptionsRef.current.get(topic);
        if (callbacks) {
